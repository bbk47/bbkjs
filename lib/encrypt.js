...
Encryptor.prototype.encrypt = function (buf) {
    ...
    let fillBytes = crypto.randomBytes(this.fillLen);
    return Buffer.concat([fillBytes, encrypted, result]);
};

Encryptor.prototype.decrypt = function (buf) {
    if (this.fillLen) {
        buf = buf.slice(this.fillLen);
    }
...
// const worker = new Encryptor('csii2019', 'aes-256-cbc', 8);
