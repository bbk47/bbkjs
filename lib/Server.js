...
class Server {
    ...
    handleConnection(type, transport) {
        transport.setSerializer(this.$serializer);
        transport.on('stream', this.handleStream.bind(this, transport));
        transport.on('error', this.handleConnError.bind(this, transport));
        transport.on('close', this.handleConnClose.bind(this, transport));
    }
    handleConnError(transport, err) {
        this.logger.error(`fire event[error] on client!message:${err.message}`);
    }
    handleConnClose(transport, code) {
        this.logger.error(`fire event[close] on client!code:${code}`);
    }
    handleStream(transport, stream, addrData) {
