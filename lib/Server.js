...
class Server {
    ...
    handleConnection(type, tunconn) {
        var eventsListen = [this.handleConnMessage.bind(this, tunconn), this.handleConnError.bind(this, tunconn), this.handleConnClose.bind(this, tunconn)];
        tunconn.bindEvents(...eventsListen);
    }
    handleConnMessage(tunconn, packet) {
        try {
            ...
            if (frame.type === protocol.PING_FRAME) {
                ...
                this.dispatchRequestFrame(tunconn, frame);
            }
        } catch (error) {
            console.log(error);
    ...
    dispatchRequestFrame(tunconn, frame) {
    ...
    flusResponseFrame(tunconn, frame) {
        frameSegment(frame, (tinyframe) => {
            try {
                const encData = this.$serializer.serialize(tinyframe);
                this.logger.debug(`write tunnel cid:${frame.cid}, data[${encData.length}]bytes`);
                tunconn.sendPacket(encData);
            } catch (err) {
