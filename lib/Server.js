...
Server.prototype.initServer = function () {
    const self = this;
    const wss1 = new WebSocket.Server({ noServer: true, perMessageDeflate: false, backlog: MAX_CONNECTIONS });
    wss1.on('connection', self.handleConnection.bind(self));
    const server = http.createServer(function (req, res) {
        res.end('helloworld');
    });
    server.on('upgrade', function upgrade(request, socket, head) {
        const { pathname } = parse(request.url);
        if (pathname === self.websocketPath) {
...
Server.prototype.handleConnection = function (connection) {
    ...
    connection.on('message', (data) => {
        try {
            const frame = self.$serializer.derialize(data);
            self.logger.debug(`read. ws tunnel cid:${frame.cid}, data[${data.length}]bytes`);
