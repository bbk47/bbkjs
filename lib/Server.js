...
function Server(config) {
    const uri = `${config.listenAddr}:${config.listenPort}${config.websocketPath}`;
...
Server.prototype.sendResponseFrame = function (client, frame) {
    const binaryData1 = meta.serialize(frame);
...
Server.prototype.dispatchRequestFrame = function (connection, frame) {
    ...
    if (frame.type === meta.INIT_FRAME) {
        ...
        targetSocket.setTimeout(5000);
        targetSocket.on('timeout', () => {
            self.logger.error(`fire event[timeout(5s)] on target[${addrInfo.dstAddr}:${addrInfo.dstPort}]!`);
            self.cleanTargetCache(targetObj, frame);
            const respFrame = { cid: frame.cid, type: meta.RST_FRAME, data: Buffer.from([0, 3]) };
            self.flusResponseFrame(respFrame);
        });
