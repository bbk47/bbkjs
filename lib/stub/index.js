...
class StubWorker extends EventEmitter {
    ...
    createStream(streamId, addr) {
        ...
        (cid, delta) => {
            // console.log('send window update frame===',cid,delta);
            const deltaBuf = Buffer.from([delta>>16,delta>>8,delta&0xff]);
            const frame = { cid, type: protocol.WINDOW_UPDATE_FRAME, data:deltaBuf };
    ...
    dataListener(packet) {
        try {
            ...
            if (frame.type === protocol.PING_FRAME) {
                ...
                const stream = this._streams[frame.cid];
                const dataBuf = frame.data;
                const delta = dataBuf[0]*256*256+dataBuf[1]*256+dataBuf[2];
                // console.log('WINDOW_UPDATE_FRAME====',frame.cid, delta);
                if (stream && stream.send_window !== undefined) {
                    // console.log('stream update SendWindow==',frame.cid,frame.delta);
