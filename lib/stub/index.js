...
class StubWorker extends EventEmitter {
    ...
    _scheduleSendLoop() {
        ...
        const sendNext = () => {
            ...
            let hasMore = false;
            Object.values(this._streams).forEach(st=>{
                if(st.sendQueue.length>0){
                    const frame = st.sendQueue.shift();
            ...
            if(hasMore){
                this._scheduleSendLoop();
            }
        };

        this._scheduleTimer = setImmediate(sendNext);
    }
    ...
    sendFrame(frame) {
        const st = this._streams[frame.cid];
        if(!st){
            this._sendFrame(frame);
        }else{
            st.sendQueue.push(frame);
            this._scheduleSendLoop();
        }
    }
    bindEvents() {
    ...
    createStream(streamId, addr) {
        ...
        function(length){
            self.sendFrame({ cid: streamId, type: protocol.WINDOW_UPDATE_FRAME, data: Buffer.from([length >> 24, length >> 16, length >> 8, length & 0xff]) });
        });
        duplex_stream.cid = streamId;
        duplex_stream.addr = addr;
        duplex_stream.sendQueue=[];
