...
// SOCKS5 UDP 数据报格式: RSV(2)+FRAG(1)+ATYP(1)+ADDR(var)+PORT(2)+DATA
function parseSocks5UdpDatagram(buf) {
    const atyp = buf[3];
    let addrEnd;
    if (atyp === 0x01) {
        addrEnd = 3 + 1 + 4 + 2; // IPv4
    } else if (atyp === 0x03) {
        addrEnd = 3 + 1 + 1 + buf[4] + 2; // domain
    } else if (atyp === 0x04) {
        addrEnd = 3 + 1 + 16 + 2; // IPv6
    } else {
...
class Client {
    ...
    handleProxyConn(isConnect, cSocket) {
        ...
        const onUdpAssociate = async (udpSocket, ctrlSocket) => {
            try {
                await this.setupEnv();
            } catch (_err) {
                udpSocket.close();
            ...
            let browserAddr = null;

            const cid = this._stubclient.startUdpSession((addrBuf, payload) => {
                if (!browserAddr) return;
                // RSV(2) + FRAG(1) + socks5_addr + payload
                const header = Buffer.concat([Buffer.from([0x00, 0x00, 0x00]), addrBuf]);
                const reply = Buffer.concat([header, payload]);
                udpSocket.send(reply, browserAddr.port, browserAddr.address, (err) => {
            ...
            udpSocket.on('message', (msg, rinfo) => {
                ...
                try {
                    ...
                    this.logger.warn('udp parse error: ' + err.message);
                }
            });

            const cleanup = () => {
                this._stubclient && this._stubclient.closeUdpSession(cid);
