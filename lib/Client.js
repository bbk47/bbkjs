...
class Client {
    ...
    initSocks5Server() {
        ...
        server.on('connection', this.handleProxyConn.bind(this, null));
        server.on('close', () => {
            this.logger.log('server is closed');
        });
        server.listen(this.listenPort, this.listenAddr, () => {
            this.logger.info('client is listening on sockss://' + this.listenAddr + ':' + this.listenPort);
        });
    }
    initHttpServer() {
        // console.log('initHttpServer...');
        const httpserver = http.createServer();
        httpserver.on('connect', this.handleProxyConn.bind(this));
        httpserver.listen(this.listenHttpPort, this.listenAddr, () => {
            this.logger.info('client is listening on https://' + this.listenAddr + ':' + this.listenHttpPort);
        });
    }
    ...
    handleProxyConn(cReq, socket) {
        ...
        cReq ? proxy.createConnectProxy(cReq, socket, ...proxyEvents) : proxy.createSocks5Proxy(socket, ...proxyEvents);
    }
