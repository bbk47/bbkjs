...
Client.prototype.handleConnection = function (socket) {
    ...
    socket.on('end', function () {
        console.info(`end event of local connection has been triggered`);
        stage = 'DESTORYED';
        self.flushRemoteFrame({ cid: connectionId, type: meta.FIN_FRAME, data: Buffer.from([0, 1]) });
    });
    socket.on('close', function (hadError) {
        console.info(`close event of local connection has been triggered!hadError:${hadError}`);
        stage = 'DESTORYED';
        socket.destroy();
        self.flushRemoteFrame({ cid: connectionId, type: meta.FIN_FRAME, data: Buffer.from([0, 1]) });
    });
    socket.on('error', function (error) {
        console.info(`error event of local connection has been triggered!message:${error.message}`);
        stage = 'DESTORYED';
        socket.destroy();
        self.flushRemoteFrame({ cid: connectionId, type: meta.FIN_FRAME, data: Buffer.from([0, 1]) });
    });
...
Client.prototype.setupWsConnection = function () {
    const self = this;
    console.log(`connecting websocket url:  [${self.websocketUrl}]`);
