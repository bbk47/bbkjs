...
class Client {
    ...
    setupTunnel() {
        ...
        if (tunnelOpts.protocol === 'ws') {
            tsport = tscreater.createWebsocketTransport(params, onOpen);
        } else if (tunnelOpts.protocol === 'h2') {
            tsport = tscreater.createHttp2Transport(params, onOpen);
        } else if (tunnelOpts.protocol === 'tls') {
            tsport = tscreater.createTlsTransport(params, onOpen);
        } else if (tunnelOpts.protocol === 'tcp') {
            tsport = tscreater.createTcpTransport(params, onOpen);
        } else {
            this._tunnelStatus = TUNNEL_INIT;
