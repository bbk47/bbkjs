    ...
    ws.on('message', function (data) {
        try {
            ...
            if (frame.type === meta.PONG_FRAME) {
                ...
                self.logger.debug(`websocket connection health:${ws.healthy}ms`);
                return;
            }
            self.flushLocalFrame(frame);
        } catch (error) {
            self.logger.fatal(`receive message  falied from server! protocol error!`);
        }
    });
    ws.on('close', function (event) {
        self.logger.debug(`close event of websocket connection has been triggered!`, event);
