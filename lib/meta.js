...
function serialize(frame) {
    ...
    let randBuf = crypto.randomBytes(6);
    let fillBytes = crypto.randomBytes(fillLen);
    const result = Buffer.concat([fillBytes, cidBuf, randBuf, typeBuf, frame.data]);
...
exports.setupEncWorker = function(password,method){
    encryptWorker = new Encryptor(password, method);
}
